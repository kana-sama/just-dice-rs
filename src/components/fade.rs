...
impl Fade {
    pub fn new(theme: ThemeRef) -> Self {
        Self {
            progress: Animated::new(easing::LINEAR, FADE_DURATION, 0., 1.),
            prev_progress: 0.,

            image: Bitmap::empty(),
            sprite: {
                let sprite = Sprite::new();
                sprite.move_to(pd::display::width() as f32 / 2., pd::display::height() as f32 / 2.);
    ...
    pub fn update(&mut self) {
        ...
        if changed {
            self.image = Bitmap::new(pd::display::width(), pd::display::width(), Color::CLEAR).unwrap().render(|_| {
                ...
                let offset = (pd::display::width() as f32 - diameter) / 2.;
    ...
    pub fn enter(&mut self) {
        self.progress = Animated::new(easing::LINEAR, 200, 0., 1.);
        self.progress.start();
    }

    pub fn leave(&mut self) {
        self.progress = Animated::new(easing::LINEAR, FADE_DURATION - self.progress.remaining_duration(), self.progress.value(), 0.);
        self.progress.start();
    }
}
